...
fn seeded_contime_one_lane(event_count: usize) -> BenchContime {
    ...
    for index in 0..event_count {
        contime.apply([new_event(0, index as u128, index as i64)].map(Into::into)).expect("seed event should apply");
    }
...
fn seeded_contime_many_lanes(lane_count: usize, events_per_lane: usize) -> BenchContime {
    ...
    for lane_id in 0..lane_count as u128 {
        for offset in 0..events_per_lane as u128 {
            ...
            contime.apply([new_event(lane_id, event_id, offset as i64)].map(Into::into)).expect("seed event should apply");
        }
    }
...
fn seeded_contime_single_event_lane() -> BenchContime {
    let contime = BenchContime::with_history_horizon(1, MEMORY_BUDGET_BYTES, 10_000);
    contime.apply([new_event(0, 0, 0)].map(Into::into)).expect("seed event should apply");
    contime
}
