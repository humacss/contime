...
fn event(time: u64) -> Event {
    Event { time: Time(time), id: time.into(), snapshots: vec![1] }
}
...
impl ApplyEvents<Event> for State {
    ...
    fn apply_events(&mut self, batch: ApplyBatch<'_, '_, Time, Event>) {
        self.values.extend(batch.events.map(|event| event.time.0));
    }
}
...
fn config(routers: usize, workers: usize, retention: u64) -> ConTimeConfig<Time> {
    ConTimeConfig {
...
fn future_history_can_be_queried_without_advancing_or_publishing_effects() {
    ...
    assert_eq!(core.query_at(Time(20), [1]).unwrap()[0].values, [10, 20]);
    assert!(effects.is_empty());
    core.advance_to(Time(10)).unwrap();
    core.wait_until_idle(Duration::from_secs(2)).unwrap();
    assert_eq!(effects.try_iter().collect::<Vec<_>>(), [10]);
    core.advance_to(Time(20)).unwrap();
    core.wait_until_idle(Duration::from_secs(2)).unwrap();
...
fn an_active_application_keeps_causal_work_admissible_while_pruning_waits() {
    ...
    core.apply([event(10), event(20)]).unwrap();
    core.advance_to(Time(100)).unwrap();
    ...
    core.apply_internal(Time(10), [event(11)]).unwrap();
    ...
    assert_eq!(core.query_at(Time(100), [1]).unwrap()[0].values, [10, 11, 20]);
    assert!(core.errors().is_empty());
    core.apply_internal(Time(10), [event(12)]).unwrap();
...
fn accepted_fanout_is_preserved_across_multi_router_pruning() {
    ...
    core.apply((1..=50).map(|time| Event { time: Time(time), id: time.into(), snapshots: (0..16).collect() })).unwrap();
    core.advance_to(Time(100)).unwrap();
    core.wait_until_idle(Duration::from_secs(3)).unwrap();
    let states = core.query_at(Time(100), 0..16).unwrap();
...
fn live_callbacks_submit_cross_worker_causal_work_before_their_reports() {
        ...
        .set(Box::new(move |time| {
            if time < 20 {
                weak.upgrade()
                    ...
                    .apply_internal(
                        Time(time),
                        [Event { time: Time(time + 1), id: (time + 1).into(), snapshots: vec![((time + 1) % 4).into()] }],
                    )
                    .unwrap();
            }
        }))
    ...
    let mut values = core.query_at(Time(100), 0..4).unwrap().into_iter().flat_map(|state| state.values).collect::<Vec<_>>();
